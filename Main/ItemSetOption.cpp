...
int CSItemOption::InitSetOptionsTooltip(ObjectItem * lpItem, int CurrentLine, char a4)
{
	...
	for (int n = 0; n <= INVENTORY_WEAR_SIZE; n++)
	{
		if (lpInfo->option[n] != -1 && lpInfo->value[n] != -1)
		{
			...
			//Fix by Lauis
			wsprintf(TooltipLine[NextLine], gCustomMessage.GetMessageB(this->GetAncientTextIndex(lpInfo->option[n])), (unsigned char)lpInfo->value[n]);
			//--
