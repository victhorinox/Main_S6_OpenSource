...
void (WINAPI *_glEnable_original)(GLenum cap) = glEnable;
void (WINAPI *_glClearColor_original)(GLclampf r,GLclampf g,GLclampf b,GLclampf a) = glClearColor;
// ----------------------------------------------------------------------------------------------

void WINAPI _glEnable_hook(GLenum mode)
{
	// ---
	if(Anisotropy != 0)
	{
	...
	_glEnable_original(mode);
}

void WINAPI _glClearColor_hook(GLclampf r,GLclampf g,GLclampf b,GLclampf a)
{
	if(gFog.EnableFog && Fog && *(DWORD*)(MAIN_SCREEN_STATE) == GameProcess)
	{
		...
		if (lpFog != 0)
		{
			...
			_glClearColor_original(rgba[0],rgba[1],rgba[2],rgba[3]);
		}
	}
}
...
void Graphics::InitOGLHook()
{
	...
	DetourAttach(&(PVOID &)_glEnable_original,_glEnable_hook);
	DetourTransactionCommit();

	//---- glClearColor
	DetourTransactionBegin();
	DetourUpdateThread(GetCurrentThread());
	DetourAttach(&(PVOID &)_glClearColor_original,_glClearColor_hook);
	DetourTransactionCommit();
}