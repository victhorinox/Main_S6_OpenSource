...
void User::Refresh()
{
	...
	for(int n=0;n < 400;n++)
	{
		...
		//gInterface.DrawItem(gObjUser.m_Logo[n].X,gObjUser.m_Logo[n].Y-30,4,4,teste,SET_ITEMOPT_LEVEL(0),0,0,1);

		teste = teste++;
	}

	//gInterface.DrawFormat(eGold,  80, 80, 45, 1, "%f %f", pFrameSpeed1, pFrameSpeed2);
	//gInterface.DrawFormat(eGold,  80, 90, 65, 1, "%d", gObjUser.GetActiveSkill());
...
/*	if( (lpPlayer->Class & 7) != 6 )
	...
	//->Fix Agility Visual Bug
	SetByte((DWORD)(0x00649E24+3),14); //Fix Agi
	*/
}
