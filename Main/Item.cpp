...
void ItemModelLoad() // OK
{
	...
	{
		LoadItemModel((it->second.ItemIndex + ITEM_BASE_MODEL),"Custom\\Pet\\",it->second.ModelName,-1);
	}
}
...
void ItemTextureLoad() // OK
{
	...
	for(std::map<int,CUSTOM_PET_INFO>::iterator it = gCustomPet2.m_CustomPetInfo.begin();it != gCustomPet2.m_CustomPetInfo.end();it++)
	{
		LoadItemTexture((it->second.ItemIndex + ITEM_BASE_MODEL),"Custom\\Pet\\",it->second.ModelName,-1);
	}
}
...
void LoadItemTexture(int index,char* folder,char* name,int value) // OK
{
	...
	//-> Fix cEffect Anti Aliasing
	((void(*)(int,char*,int,int,int))0x00614710)(index,folder,0x2900,0x2600,0x01);
}
