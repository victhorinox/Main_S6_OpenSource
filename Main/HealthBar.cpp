...
{
	if(gProtect.m_MainInfo.MonsterLifeBarType == 0)
	{
		...
		for(int n=0;n < MAX_MAIN_VIEWPORT;n++)
		{
			...
			{
				pDrawBarForm((float)(i * 4 + PosX + 0), (float)(PosY + 2), 3.0, 2.0, 0.0, 0);
			}
			...
			if (ShieldPercent>0) 
			{
				...
				{
					pDrawBarForm((float)(i * 4 + PosX + 0), (float)(PosY + 6), 3.0, 2.0, 0.0, 0);
				}
			}
	...
	if(gProtect.m_MainInfo.MonsterLifeBarType == 1)
	{
		int PosX, PosY, LifeProgress, ShieldProgress;
		...
		for(int n=0;n < MAX_MAIN_VIEWPORT;n++)
		{
			...
			{
				pSetBlend(true);
				glColor4f((GLfloat)0.0, (GLfloat)0.0, (GLfloat)0.0, (float)0.5);
				pDrawBarForm((float)(PosX - 3), (float)(PosY), LifeBarWidth + 6.0f, 10.0f, 0.0f, 0);
				pGLSwitchBlend();
				pGLSwitch();
				glColor3f(1.0, 1.0, 1.0);
				pSetBlend(false);
			...
			else
			{
				pSetBlend(true);
				glColor4f((GLfloat)0.0, (GLfloat)0.0, (GLfloat)0.0, (float)0.5);
				pDrawBarForm((float)(PosX - 5), (float)(PosY - 10), LifeBarWidth + 10.0f, 18.0f, 0.0f, 0);
				pGLSwitchBlend();
				pGLSwitch();
				glColor3f(1.0, 1.0, 1.0);
				pSetBlend(false);

				gCustomRankUser.DrawFormat(eWhite, PosX, PosY - 8, LifeBarWidth, 3, "%s", name);
			}
