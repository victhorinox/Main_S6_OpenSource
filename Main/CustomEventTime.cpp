...
void CCustomEventTime::GCReqEventTime(PMSG_CUSTOM_EVENTTIME_RECV* lpMsg) // OK
{
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		if (this->gCustomEventTime[n].index >= 32 && this->gCustomEventTime[n].time != -1) this->Arena = 1;
	}
...
void CCustomEventTime::DrawEventTimePanelWindow()
{
	...
	StartY = gInterface.DrawRepeatGUI(eVip_FRAME, StartX, StartY + (float)40.0, 12);
	...
	if ((this->Page == 0) || (this->Page == 1) || (this->Page == 2 && this->Arena==1))
	{
	...
	gInterface.DrawFormat(eGold, (int)StartX + 10, (int)StartBody + 10, 210, 3, gCustomMessage.GetMessage(25));
	...
	if (this->EventTimeEnable == 1)
	{
		...
		if (this->Page == 0)
		{
			gInterface.DrawFormat(eGold,  StartX + 50, StartBody + 37, 52, 1,"Event");
		}

		if (this->Page == 1)
		{
			gInterface.DrawFormat(eGold,  StartX + 50, StartBody + 37, 52, 1,"Invasion");
		}

		if (this->Page == 2)
		{
			gInterface.DrawFormat(eGold,  StartX + 50, StartBody + 37, 52, 1,"Boss");
		}

		if (this->Page == 3)
		{
			gInterface.DrawFormat(eGold,  StartX + 50, StartBody + 37, 52, 1,"Arena");
		}

		gInterface.DrawFormat(eGold,  StartX + 150, StartBody + 37, 52, 1,"Time");
		...
		for( int i = (15*Page)+0; i < (15*Page)+15; i++ )
		{
			...
			gInterface.DrawFormat(eWhite,  StartX + 50, StartBody + 55 + (line), 52, 1,this->m_CustomEventInfo[i].Name);

			gInterface.DrawFormat(Color,  StartX + 150, StartBody + 55 + (line), 52, 1,text2);
